...
(()=>{
  ...
  async function code3301(input){
    ...
    await fbDb.runTransaction(async tx=>{const snap=await tx.get(ref);if(!snap.exists)throw Error('PROFILE_NOT_FOUND');const profile=snap.data()||{},base=normalizeBase(profile),id='secret_3301_lr3000';if(base.redeemedCodes[id])throw Error('CODE_ALREADY_USED');base.redeemedCodes[id]=new Date().toISOString();base.lr+=3000;tx.set(ref,{nick:base.nick||profile.nick||'',lr:base.lr,lp:base.lp,data:base,updated_at:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});out=base});
  ...
  async function secretBadge(input){
    ...
    await fbDb.runTransaction(async tx=>{const snap=await tx.get(ref);if(!snap.exists)throw Error('PROFILE_NOT_FOUND');const profile=snap.data()||{},base=normalizeBase(profile);if(!futureDone(base))throw Error('FUTURE_NOT_CLEARED');if(base.badges[BADGE_ID]){already=true;out=base;return}base.badges[BADGE_ID]=Date.now();if(!base.missions.badgePaid[BADGE_ID]){base.missions.badgePaid[BADGE_ID]=true;base.lr+=10}tx.set(ref,{nick:base.nick||profile.nick||'',lr:base.lr,lp:base.lp,data:base,updated_at:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});out=base});
